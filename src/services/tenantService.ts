...
export const tenantService = {
  ...
  async getUserTenant(userId: string): Promise<Tenant | null> {
    ...
    if (error) {
      if (error.code === 'PGRST116') return null // Pas de tenant trouvé
      throw error
    }
  ...
  async addUserToTenant(tenantId: string, userId: string, role: 'admin' | 'member' | 'viewer' = 'member'): Promise<TenantUser> {
    const { data, error } = await supabase
      .from('tenant_users')
      .insert([{
        tenant_id: tenantId,
